...
def echo_classification_biggerstaff(
        grid, refl_field, work_lev,
        intense=40, bkg_rad=11000.0, area_relation='sgp', peak_relation='sgp',
        use_intense=True, lapse_lim=3.5, grad_lim=3.0, z_melt=3000.,
        layer=0, win_size=5, fill_value=-9999.0):
    """
    Perform convective/statiform precipitation echo classification using
    the algorithm from Biggerstaff et al.
    ...
    use_inter : bool
        ...
        classify using only the peakedness metric.
    lapse_lim : float
        Unknown XXX (currently not used)
    grad_lim : float
        Unknown XXX (currently not used)
    z_melt : float
        Melting layer location in meters.
    layer : float
        Layer (XXX) location in meters.
    win_size : int
        Length of window applied to classified echos.
    fill_val : float, optional
        Masked array fill value.
    ...
    M. I. Biggerstaff and S. A. Listemaa, An Improved Scheme for
    Convective/Stratiform Echo Classification Using Radar Reflectivity.
    Journal of Applied Meteorology, 2000, 39, 2129-2150.
    ...
    # perform classification
    sclass = _echo_classification.steiner(
        x, y, z, ze, dx, dy, intense, bkg_rad, work_lev, area_relation,
        peak_relation, use_intense, fill_val=fill_value)

    eclass = _echo_classification.biggerstaff_class(
        z, ze, sclass, dx, dy, dz,
        work_level=work_lev, lapse_lim=lapse_lim, grad_lim=grad_lim,
        z_melt=z_melt, layer=layer, win_size=win_size, fill_val=fill_value)
